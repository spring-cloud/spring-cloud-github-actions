...
| [mark-branch-merged](.github/actions/mark-branch-merged/) | Records a branch as already merged into another with an `ours` merge, so the retarget commit does not travel to `main` when the line is merged forward. Verifies the tree is unchanged before pushing. | [README](.github/actions/mark-branch-merged/README.md) |
