...
| [devkit-cli](.github/actions/devkit-cli/) | Builds the [spring-devkit](https://github.com/spring-io/spring-devkit) CLI (`spd`) from source and optionally runs a command with it. | [README](.github/actions/devkit-cli/README.md) |
| [update-license-headers](.github/actions/update-license-headers/) | Replaces Apache License 2.0 headers with the Broadcom license header in the file types `spd migrate-license` does not cover — XML (including the Maven `<licenses>` block), YAML/shell, and block-comment languages devkit ignores. Java/Kotlin/Groovy/Gradle, `.properties`, `LICENSE` files and `checkstyle-header.txt` are owned by `spd migrate-license`. | [README](.github/actions/update-license-headers/README.md) |
