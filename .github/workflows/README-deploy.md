...
| `runs_on` | Runner to use for the build job. Builds that reach commercial Artifactory **must** set this — see below. | No | string (default: `ubuntu-latest`) |
...
| `timeout_minutes` | Timeout for each build job. Raise it for projects whose reactor does not finish in the default hour on a single runner. GitHub caps a job at 360. | No | number (default: `60`) |

### `runs_on` and commercial Artifactory

**Commercial Artifactory (`usw1.packages.broadcom.com`) only serves requests from runners in the `spring-enterprise-builds` group.** A GitHub-hosted runner is refused at the network layer with **`403 Forbidden`** — before authentication is even attempted, and therefore *regardless of whether the credentials are correct*.

This matters for any branch carrying `release-ci-settings.xml`, i.e. `<major>.<minor>.x-internal` and `release/<version>` branches, whose dependencies resolve from commercial Artifactory. Those builds must pass a commercial runner label, currently `ubuntu22-2-8`. The `ci-release.yml` generated by [`add-commercial-release-files`](../actions/add-commercial-release-files/action.yml) does this already.

The failure is easy to misread, so it is worth knowing the signature:

- **`403`** — the request never got past the network boundary. Almost always the wrong runner, **not** a credentials problem. Maven will still report having offered Basic auth (`Using connector ... with username=***`), which makes it look like a rejected password.
- **`401`** — genuinely about credentials. Note that a *scattering* of 401s in a healthy build is normal: Maven does not send credentials preemptively, so the first request to a repository is anonymous and expects a 401 challenge before retrying authenticated.

A project delegating to its own reusable workflow rather than to `deploy.yml` has to thread the runner through itself — see the `runs-on` input on `spring-cloud-kubernetes`'s `maven.yaml`.
